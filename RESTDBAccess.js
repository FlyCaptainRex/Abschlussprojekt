...
app.post("/orders", (req, res) => {
  ...
  const userSql =
    "INSERT INTO user (first_name, last_name, country, zip_code, street, city, email) VALUES (?, ?, ?, ?, ?, ?, ?)"; // SQL-Abfrage zum Einfügen eines neuen Benutzers
  const userValues = [
    user.first_name,
    user.last_name,
    user.country,
  ...
  con.query(userSql, userValues, (err, userResult) => {
    ...
    con.query(orderMainSql, [userId], (err, orderMainResult) => {
      ...
      const orderDetailSql =
        "INSERT INTO order_detail (order_id, perfume_name, brand, perfume_price, perfume_amount) VALUES ?"; // SQL-Abfrage zum Einfügen von Bestelldetails
      const orderDetailValues = cart.map((item) => [
        orderId,
        item.name,
        item.brand,
        item.price,
        item.quantity,
      ]); // Werte für order_detail
      ...
      con.query(orderDetailSql, [orderDetailValues], (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "Error inserting order_detail" });
        }
